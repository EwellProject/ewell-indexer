...
public class ProxyAccountManagementAddressAddedProcessor : AElfLogEventProcessorBase<ProxyAccountManagementAddressAdded, LogEventInfo>
{
    ...
    private readonly ILogger<AElfLogEventProcessorBase<ProxyAccountManagementAddressAdded, LogEventInfo>> _logger;

    public ProxyAccountManagementAddressAddedProcessor(
    ...
    protected override async Task HandleEventAsync(ProxyAccountManagementAddressAdded eventValue,
        LogEventContext context)
    {
        _logger.LogInformation(
            "[ProxyAccountManagementAddressAdded] handle chainId {chainId} proxyAccountAddress {proxyAccountAddress}",
        ...
        {
            _logger.LogInformation(
                "[ProxyAccountManagementAddressAdded] proxyAccountIndexId {proxyAccountIndexId} not exist.",
                proxyAccountIndexId);
            return;
        }

        _objectMapper.Map(context, proxyAccountIndex);
        proxyAccountIndex.ManagersSet.Add(eventValue.ManagementAddress.Address.ToBase58());
